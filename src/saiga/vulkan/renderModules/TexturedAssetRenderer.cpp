...
// deferred renderer
void DeferredTexturedAssetRenderer::destroy()
{
    Pipeline::destroy();
    uniformBufferVS.destroy();
}

void DeferredTexturedAssetRenderer::bindTexture(vk::CommandBuffer cmd, vk::DescriptorSet ds)
{
    cmd.bindDescriptorSets(vk::PipelineBindPoint::eGraphics, pipelineLayout, 0, ds, nullptr);
}

void DeferredTexturedAssetRenderer::pushModel(vk::CommandBuffer cmd, mat4 model)
{
    pushConstant(cmd, vk::ShaderStageFlagBits::eVertex, sizeof(mat4), data(model));
}
...
void DeferredTexturedAssetRenderer::updateUniformBuffers(vk::CommandBuffer cmd, mat4 view, mat4 proj)
{
    uboVS.projection = proj;
    uboVS.modelview  = view;
    uboVS.lightPos   = vec4(5, 5, 5, 0);
...
void DeferredTexturedAssetRenderer::init(VulkanBase& vulkanDevice, VkRenderPass renderPass,
...
StaticDescriptorSet DeferredTexturedAssetRenderer::createAndUpdateDescriptorSet(Texture& texture)
{
    //    vk::DescriptorSet descriptorSet = device.allocateDescriptorSets(
    //                vk::DescriptorSetAllocateInfo(descriptorPool,descriptorSetLayout.size(),descriptorSetLayout.data())
    //                )[0];
