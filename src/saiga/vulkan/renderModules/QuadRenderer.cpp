...
void QuadRenderer::updateUniformBuffers(vk::CommandBuffer cmd, mat4 proj, mat4 view, vec4 lightColor,
                                        vec4 lightDirection, bool debug, float intensity)
{
    uboVS.proj       = proj;
    uboVS.view       = view;
    uboVS.lightDir   = lightDirection;
    uboVS.lightColor = lightColor;
    uboVS.debug      = debug;
