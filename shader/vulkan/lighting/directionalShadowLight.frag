...
void main()
{
    ...
    vec3 L = -normalize(
        mat3(ubo.view) *
        pushConstants.lightDirection.xyz);  // normalize(vec3(pushConstants.model[2])));//viewLightPos.xyz - P.xyz;
    ...
    if (fragmentInShadowMap)
        outColor =
            vec4(ambient + filterPCF(vLight, shadowmap, pushConstants.ambientIntensity) * (diffuse + specular), 1.f);
